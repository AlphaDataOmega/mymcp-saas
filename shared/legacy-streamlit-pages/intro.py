...
from streamlit_pages.header import show_header

def intro_tab():
    """Display the introduction and setup guide for MyMCP.me"""
    
    # Show the cool header
    show_header()
    
    # Welcome message with browser automation focus
    st.markdown("""
    **MyMCP.me** combines the proven agent-building capabilities of Archon with powerful browser automation tools from BrowserMCP. 
    
    Create intelligent AI agents that can not only generate code but also interact with web pages, fill forms, click buttons, 
    take screenshots, and perform complex web-based tasks - all through natural language commands.
    
    🚀 **Key Features:**
    - **Intelligent Agent Generation**: Build AI agents using proven multi-agent workflows
    - **Browser Automation**: Control web browsers with natural language 
    - **MCP Integration**: Connect to multiple MCP servers and tools
    - **Visual Feedback**: See what your agents are doing with screenshots and page snapshots
    - **Tool Library**: Access pre-built tools and automation capabilities
    """)
    
    # Browser automation showcase
    with st.expander("🌐 Browser Automation Examples", expanded=True):
        st.markdown("""
        **What can your agents do with browser automation?**
        
        ```python
        # Navigate and interact with web pages
        navigate_to_url("https://example.com")
        click_element("Sign Up")
        type_text("email", "user@example.com")
        select_dropdown_option("country", "United States")
        press_key("Enter")
        take_screenshot()  # Visual confirmation
        ```
        
        **Real-world use cases:**
        - 📊 **Data Collection**: Scrape information from multiple websites
        - 🛒 **E-commerce**: Automated shopping and price monitoring  
        - 📋 **Form Filling**: Bulk form submissions and registrations
    ...
    st.info("""
    **✨ NEW:** MyMCP.me includes browser automation tools alongside traditional AI agent capabilities. 
    The setup process is similar to Archon but with additional browser automation features.
    """)
