...
export class ServerManager {
  ...
  async installServer(config: ServerInstallConfig): Promise<string> {
    ...
    try {
      ...
      const agentId = await agentRegistry.registerAgent({
        ...
        description: `MCP server wrapper for ${config.name} with ${config.tools.length} tools: ${config.tools.join(', ')}`,
        code: agentCode,
        tools: config.tools,
        metadata: {
  ...
  private generateServerAgentCode(server: InstalledServer): string {
    const toolFunctions = server.tools.map(toolName => `
@server_agent.tool
...
Provides individual tools: ${server.tools.join(', ')}
...
server_agent = Agent(
    ...
    system_prompt="""You are a ${server.name} server agent that provides access to ${server.tools.length} tools.
