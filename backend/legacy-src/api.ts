...
app.get("/tools", async (req, res) => {
  try {
    ...
    // Get base tools
    const baseTools = snapshotTools.map(tool => ({
    ...
    // Get local tools from agent-resources
    const localTools = localToolManager.getAllTools().map(tool => {
      const localTool = localToolManager.generateMCPTool(tool.id);
      return {
        name: localTool.schema.name,
        description: localTool.schema.description,
        inputSchema: localTool.schema.inputSchema
      };
    });
    
    console.log(`📋 Serving ${baseTools.length} base tools + ${agentTools.length} agent tools + ${customTools.length} custom tools + ${localTools.length} local tools`);
    
    res.json({ tools: [...baseTools, ...agentTools, ...customTools, ...localTools] });
  } catch (error) {
