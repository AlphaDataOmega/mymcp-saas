...
export class LocalToolManager {
  ...
  constructor() {
    ...
    // Check environment variable to enable/disable local tools
    this.enabled = process.env.ENABLE_LOCAL_TOOLS !== 'false';
    
    if (this.enabled) {
      this.scanForTools();
    } else {
      console.log('📁 Local tools disabled via ENABLE_LOCAL_TOOLS=false');
    }
  }
  ...
  getEnabledTools(): LocalTool[] {
    if (!this.enabled) return [];
    return Array.from(this.tools.values()).filter(tool => !this.disabledTools.has(tool.id));
  }
  ...
  toggleTool(toolId: string, enabled: boolean): boolean {
    if (!this.enabled || !this.tools.has(toolId)) return false;
